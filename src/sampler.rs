...
fn rand_f32()->f32{
    let s=SEED.fetch_update(Ordering::Relaxed,Ordering::Relaxed,
        |v|Some(v.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407))).unwrap();
    ((s>>33) as f32)/(u32::MAX as f32)
}
pub fn sample(logits:&mut Vec<f32>,temp:f32)->usize{
    if temp<=0.0{return logits.iter().enumerate()
        .max_by(|a,b|a.1.partial_cmp(b.1).unwrap()).map(|(i,_)|i).unwrap_or(0);}
    for v in logits.iter_mut(){*v/=temp;}
    let m=logits.iter().cloned().fold(f32::NEG_INFINITY,f32::max);
    let mut s=0f32;
    for v in logits.iter_mut(){*v=(*v-m).exp();s+=*v;}
