...
fn rstr(r:&mut impl Read)->anyhow::Result<String>{
    let n=ru64(r)? as usize; let mut b=vec![0u8;n]; r.read_exact(&mut b)?;
    Ok(String::from_utf8_lossy(&b).into_owned())
}
fn rval(r:&mut impl Read,tid:u32)->anyhow::Result<GgufValue>{
    Ok(match tid{
        0=>GgufValue::U8(ru8(r)?),    1=>GgufValue::I8(ru8(r)? as i8),
        2=>GgufValue::U16(ru16(r)?),  3=>GgufValue::I16(ru16(r)? as i16),
        4=>GgufValue::U32(ru32(r)?),  5=>GgufValue::I32(ri32(r)?),
        6=>GgufValue::F32(rf32(r)?),  7=>GgufValue::Bool(ru8(r)?!=0),
        8=>GgufValue::String(rstr(r)?),
        9=>{let et=ru32(r)?;let n=ru64(r)? as usize;
            let mut a=Vec::with_capacity(n);for _ in 0..n{a.push(rval(r,et)?);}GgufValue::Array(a)}
        10=>GgufValue::U64(ru64(r)?), 11=>GgufValue::I64(ri64(r)?),
...
pub fn parse<R:Read+Seek>(mut r:R)->anyhow::Result<GgufFile>{
    ...
    for _ in 0..n_kv{
        let key=rstr(&mut r)?; let tid=ru32(&mut r)?;
        let val=rval(&mut r,tid).with_context(||format!("key '{}'",key))?;
        metadata.insert(key,val);
    }
    let mut tensors=Vec::with_capacity(n_tensors);
    for _ in 0..n_tensors{
        let name=rstr(&mut r)?; let nd=ru32(&mut r)? as usize;
        let mut dims=Vec::with_capacity(nd);
        for _ in 0..nd{dims.push(ru64(&mut r)?);}
