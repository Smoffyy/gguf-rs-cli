...
impl Tokenizer {
    pub fn from_gguf(gguf: &GgufFile) -> anyhow::Result<Self> {
            ...
            for (rank, v) in arr.iter().enumerate() {
                if let Some(s) = v.as_str() {
                    if let Some(sp) = s.find(' ') {
                        merge_rank.insert((s[..sp].to_string(), s[sp+1..].to_string()), rank);
                    }
                }
            }
        }
        ...
        Ok(Self { vocab, scores, token_to_id, merge_rank, byte_enc, byte_dec,
    ...
    fn encode_seg(&self, text: &str, ids: &mut Vec<u32>) {
        ...
        let mut syms: Vec<String> = match self.tok_model {
            ...
            TokModel::Gpt2  => text.bytes()
                .map(|b| self.byte_enc[b as usize].to_string()).collect(),
        };
        match self.tok_model {
            TokModel::Llama => self.bpe_score(&mut syms),
            TokModel::Gpt2  => self.bpe_rank(&mut syms),
        }
        for sym in &syms {
            if let Some(&id) = self.token_to_id.get(sym) { ids.push(id); }
            else {
                for b in sym.as_bytes() {
                    let k = format!("<0x{:02X}>", b);
                    if let Some(&id) = self.token_to_id.get(&k) { ids.push(id); }
                }
            }
        }
    }
        ...
        loop {
            ...
            for i in 0..s.len().saturating_sub(1) {
                ...
                if let Some(&id) = self.token_to_id.get(&m) {
                    ...
                    if sc > best { best = sc; bi = Some(i); }
                }
            }
            match bi {
                None    => break,
                Some(i) => { let m = format!("{}{}", s[i], s[i+1]); s[i] = m; s.remove(i+1); }
            }
        }
    }
    ...
    fn bpe_rank(&self, s: &mut Vec<String>) {
        loop {
            ...
            for i in 0..s.len().saturating_sub(1) {
                ...
                if let Some(&r) = self.merge_rank.get(&p) {
                    if r < best { best = r; bi = Some(i); }
                }
            }
            match bi {
                None    => break,
    ...
    fn split_special(&self, text: &str) -> Vec<(String, bool)> {
        ...
        if !cur.is_empty() { res.push((cur, false)); }
        res
    }
        ...
        match self.tok_model {
            ...
            TokModel::Gpt2  => {
                let bytes: Vec<u8> = s.chars()
                    .filter_map(|c| self.byte_dec.get(&c).copied()).collect();
