...
impl Tokenizer {
    ...
    // add_bos: only true for non-ChatML models (LLaMA-2 etc)
    // Qwen/ChatML templates start with <|im_start|> — no separate BOS needed
    ...
    fn encode_seg(&self,text:&str,ids:&mut Vec<u32>){
        ...
        for sym in &syms{
            ...
            else{for b in sym.as_bytes(){let k=format!("<0x{:02X}>",b);if let Some(&id)=self.token_to_id.get(&k){ids.push(id);}}}
        }
    }
    fn bpe_score(&self,s:&mut Vec<String>){
        loop{let mut best=f32::NEG_INFINITY;let mut bi=None;
    ...
    fn bpe_rank(&self,s:&mut Vec<String>){
        loop{let mut best=usize::MAX;let mut bi=None;
            for i in 0..s.len().saturating_sub(1){
