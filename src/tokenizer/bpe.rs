...
impl Tokenizer {
    pub fn from_gguf(gguf: &GgufFile) -> anyhow::Result<Self> {
        ...
        let eos_id = gguf.metadata.get("tokenizer.ggml.eos_token_id")
            .and_then(|v| v.as_u32()).unwrap_or(2);

        let tok_model = match gguf.metadata.get("tokenizer.ggml.model")
            .and_then(|v| v.as_str()) {
            Some("gpt2") => TokModel::Gpt2,
            _            => TokModel::Llama,
        };

        let byte_enc = build_byte_encoder();
        let byte_dec: HashMap<char, u8> = byte_enc.iter()
            .enumerate().map(|(b, &c)| (c, b as u8)).collect();

        // Load ranked BPE merges (GPT-2 style: lower index = merge first)
        ...
        // Identify special tokens via token_type metadata (2=control, 3=user-defined)
        ...
        Ok(Self { vocab, scores, token_to_id, merge_rank,
                  byte_enc, byte_dec, special, tok_model, bos_id, eos_id })
    }

    pub fn encode(&self, text: &str, add_bos: bool) -> Vec<u32> {
        let mut ids = if add_bos { vec![self.bos_id] } else { vec![] };
        for (seg, is_special) in self.split_special(text) {
            if is_special {
                if let Some(&id) = self.token_to_id.get(&seg) { ids.push(id); }
            } else {
                self.encode_segment(&seg, &mut ids);
            }
        }
        ids
    }
    ...
    fn encode_segment(&self, text: &str, ids: &mut Vec<u32>) {
        ...
        let mut syms: Vec<String> = match self.tok_model {
            TokModel::Llama => {
                format!(" {}", text).chars()
                    .map(|c| if c == ' ' { "\u{2581}".to_string() } else { c.to_string() })
                    .collect()
            }
            TokModel::Gpt2 => {
                text.bytes().map(|b| self.byte_enc[b as usize].to_string()).collect()
            }
        };

        match self.tok_model {
            TokModel::Llama => self.bpe_by_score(&mut syms),
            TokModel::Gpt2  => self.bpe_by_rank(&mut syms),
        }
        ...
        for sym in &syms {
            if let Some(&id) = self.token_to_id.get(sym) {
                ...
                for byte in sym.as_bytes() {
                    ...
                    if let Some(&id) = self.token_to_id.get(&key) { ids.push(id); }
                }
            }
        }
    }

    fn bpe_by_score(&self, syms: &mut Vec<String>) {
        loop {
            let mut best = f32::NEG_INFINITY;
            let mut best_i = None;
            for i in 0..syms.len().saturating_sub(1) {
                let m = format!("{}{}", syms[i], syms[i+1]);
                if let Some(&id) = self.token_to_id.get(&m) {
                    let s = self.scores[id as usize];
                    if s > best { best = s; best_i = Some(i); }
                }
            }
            match best_i {
                None    => break,
                Some(i) => { let m = format!("{}{}", syms[i], syms[i+1]); syms[i] = m; syms.remove(i+1); }
            }
        }
    }

    fn bpe_by_rank(&self, syms: &mut Vec<String>) {
        loop {
            let mut best_rank = usize::MAX;
            let mut best_i = None;
            for i in 0..syms.len().saturating_sub(1) {
                let pair = (syms[i].clone(), syms[i+1].clone());
                if let Some(&r) = self.merge_rank.get(&pair) {
    ...
    // Greedily split text on known special tokens (longest match first)
    fn split_special(&self, text: &str) -> Vec<(String, bool)> {
        ...
        if !current.is_empty() { result.push((current, false)); }
        result
    }
        ...
        match self.tok_model {
            ...
            TokModel::Gpt2  => {
                let bytes: Vec<u8> = s.chars()
                    .filter_map(|c| self.byte_dec.get(&c).copied())
                    .collect();
                if bytes.len() == s.chars().count() {
                    // All chars were in byte table — it's a normal token
                    String::from_utf8_lossy(&bytes).into_owned()
                } else {
                    // Special token string — return as-is
                    s.clone()
                }
            }
        }
...
// Build GPT-2 byte→char table (identical to Python's bytes_to_unicode())
// Printable ranges map to themselves; everything else maps to U+0100+n
fn build_byte_encoder() -> [char; 256] {
    let mut enc = ['\0'; 256];
    let mut mapped = [false; 256];
    for &(lo, hi) in &[(33usize, 126), (161, 172), (174, 255)] {
        for b in lo..=hi {
            enc[b] = char::from_u32(b as u32).unwrap();
            mapped[b] = true;
        }
    }
    let mut n = 0u32;
    for b in 0usize..256 {
        if !mapped[b] { enc[b] = char::from_u32(256 + n).unwrap_or('?'); n += 1; }
    }
    enc
}